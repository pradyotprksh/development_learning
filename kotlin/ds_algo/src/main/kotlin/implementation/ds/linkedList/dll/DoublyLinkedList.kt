...
object DoublyLinkedList {
    fun implementation() {
        ...
        dll.reverse()
        dll.printSLL() // [1]<->null. Length: 1

        dll.append(data = 2)

        dll.printSLL() // [1]<->[2]<->null. Length: 2
        ...
        dll.append(data = 5)
        dll.append(data = 6)
        dll.printSLL() // [1]<->[2]<->[3]<->[4]<->[5]<->[6]<->null. Length: 6

        dll.insertAtStart(data = 7)
        dll.printSLL() // [7]<->[1]<->[2]<->[3]<->[4]<->[5]<->[6]<->null. Length: 7

        dll.insertAtPosition(pos = 2, data = 8)
        dll.printSLL() // [7]<->[1]<->[8]<->[2]<->[3]<->[4]<->[5]<->[6]<->null. Length: 8

        dll.insertAtPosition(pos = 7, data = 9)
        dll.printSLL() // [7]<->[1]<->[8]<->[2]<->[3]<->[4]<->[5]<->[9]<->[6]<->null. Length: 9

        dll.insertAtPosition(pos = 0, data = 10)
        dll.printSLL() // [10]<->[7]<->[1]<->[8]<->[2]<->[3]<->[4]<->[5]<->[9]<->[6]<->null. Length: 10

        dll.insertAtPosition(pos = dll.length(), data = 11)
        dll.printSLL() // [10]<->[7]<->[1]<->[8]<->[2]<->[3]<->[4]<->[5]<->[9]<->[6]<->[11]<->null. Length: 11
        ...
        */
        ...
        dll.printSLL() // [1]->[2]->[3]->[4]->[5]->null. Length: 5
        ...
        dll.reverse()
        dll.printSLL() // [6]->[5]->[4]->[3]->[2]->[1]->null. Length: 6

        println("Deleting starting node ${dll.deleteStart()?.onlyNodeString()}") // Deleting node [6]->...
        dll.printSLL() // [5]->[4]->[3]->[2]->[1]->null. Length: 5

        dll.insertAtStart(6)
        dll.printSLL() // [6]->[5]->[4]->[3]->[2]->[1]->null. Length: 6

        println("Deleting last node ${dll.deleteLast()?.onlyNodeString()}") // Deleting last node [1]->...
        dll.printSLL() // [6]->[5]->[4]->[3]->[2]->null. Length: 5
        ...
        dll.printSLL() // [1]->[2]->[2]->[3]->[4]->[5]->[6]->null. Length: 7

        dll.insertAtPosition(pos = 4, data = 4)
        dll.printSLL() // [1]->[2]->[2]->[3]->[4]->[4]->[5]->[6]->null. Length: 8

        dll.insertAtPosition(pos = 5, data = 4)
        dll.printSLL() // [1]->[2]->[2]->[3]->[4]->[4]->[4]->[5]->[6]->null. Length: 9
        ...
        dll.printSLL() // [1]->[1]->[1]->[1]->[1]->null. Length: 5
        ...
        dll.printSLL() // [1]->[1]->[1]->[1]->[2]->null. Length: 5
        ...
        dll.printSLL() // [0]->[1]->[2]->[3]->[4]->[4]->[5]->[5]->[6]->[7]->[10]->null. Length: 11
