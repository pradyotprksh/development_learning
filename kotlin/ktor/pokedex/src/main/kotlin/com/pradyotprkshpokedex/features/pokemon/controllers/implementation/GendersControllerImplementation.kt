...
class GendersControllerImplementation(
    ...
    override suspend fun getDetails(context: ApplicationCall, resource: PokemonResource.Gender.Id) {
        if (resource.isValid) {
            context.respond(status = HttpStatusCode.OK, pokemonService.getGenderDetails(id = resource.id))
        } else {
            throw ParametersInvalidException(invalidParameters = listOf(Paths.Parameters.ID))
        }
    }

    override suspend fun getByPagination(context: ApplicationCall, resource: PokemonResource.Gender.Pagination) {
        if (resource.isValid) {
            val details =
